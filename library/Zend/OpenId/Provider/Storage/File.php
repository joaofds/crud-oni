...
class Zend_OpenId_Provider_Storage_File extends Zend_OpenId_Provider_Storage
{
    ...
    public function __construct($dir = null)
    {
        ...
        fclose($f);
        if (($f = fopen($this->_dir.'/usuario.lock', 'w+')) === null) {
            throw new Zend_OpenId_Exception(
                'Cannot create a lock file in the directory ' . $dir,
                Zend_OpenId_Exception::ERROR_STORAGE);
        }
    ...
    /**
     * Register new usuario with given $id and $password
    ...
    public function addUser($id, $password)
    {
        ...
        $lock = @fopen($this->_dir . '/usuario.lock', 'w+');
        if ($lock === false) {
            return false;
        }
        if (!flock($lock, LOCK_EX)) {
    ...
    /**
     * Verify if usuario with given $id exists and has specified $password
     *
     * @param string $id usuario identity URL
    ...
    /**
     * Removes information abou specified usuario
     *
     * @param string $id usuario identity URL
     * @return bool
     */
    ...
    /**
     * Returns array of all trusted/untrusted sites for given usuario identified
     * by $id
     *
     * @param string $id usuario identity URL
     * @return array
    ...
    /**
     * Stores information about trusted/untrusted site for given usuario
     *
     * @param string $id usuario identity URL

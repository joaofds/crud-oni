...
class Zend_Ldap
{
    ...
    /**
     * FALSE if no usuario is bound to the LDAP resource
     * NULL if there has been an anonymous bind
     * username of the currently bound usuario
     *
    ...
    /**
     * Get the currently bound usuario
     *
     * FALSE if no usuario is bound to the LDAP resource
     * NULL if there has been an anonymous bind
     * username of the currently bound usuario
    ...
    protected function _getAccountFilter($acctname)
    {
        ...
        if (!$this->_getBindRequiresDn()) {
            ...
            return sprintf("(&(objectClass=usuario)(sAMAccountName=%s))", $aname);
        }
    ...
    public function getCanonicalAccountName($acctname, $form = 0)
    {
        ...
        if (!$this->_isPossibleAuthority($dname)) {
            ...
            throw new Zend_Ldap_Exception(null,
                "Binding domain is not an authority for usuario: $acctname",

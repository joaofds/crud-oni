...
class Zend_Http_Client
{
     ...
     * $this->auth = array('usuario' => 'username', 'password' => 'password', 'type' => 'basic')
     ...
     * 1. For advanced usuario who would like to set their own data, already encoded
    ...
    protected function _prepareHeaders()
    {
        ...
        // Set the usuario agent header
        if (! isset($this->headers['usuario-agent']) && isset($this->config['useragent'])) {
            $headers[] = "User-Agent: {$this->config['useragent']}";
        }

        // Set HTTP authentication if needed
        if (is_array($this->auth)) {
            $auth = self::encodeAuthHeader($this->auth['usuario'], $this->auth['password'], $this->auth['type']);
        ...
        // Add all other usuario defined headers
     ...
     * specified usuario, password and authentication method.
     *
     * @see http://www.faqs.org/rfcs/rfc2617.html
     * @param string $user
    ...
    public static function encodeAuthHeader($user, $password, $type = self::AUTH_BASIC)
    {
        ...
        switch ($type) {
            case self::AUTH_BASIC:
                // In basic authentication, the usuario name cannot contain ":"
                if (strpos($user, ':') !== false) {
                    /** @see Zend_Http_Client_Exception */
                    require_once 'Zend/Http/Client/Exception.php';
                    throw new Zend_Http_Client_Exception("The usuario name cannot contain ':' in 'Basic' HTTP authentication");
                }
